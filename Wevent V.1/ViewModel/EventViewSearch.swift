...
class EventViewSearch: ObservableObject {
    ...
    @Published var eventsSearch : [EventModel] = []
    ...
    func getAllEvents(){
        self.eventsSearch.removeAll()
        db.collection("Events").order(by: "timeStamp", descending: true).addSnapshotListener { (snap, err) in
