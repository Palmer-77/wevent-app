...
class EventViewEdit: ObservableObject {
    ...
    @Published var eventsEdit : [EventModel] = []
    ...
    func getAllEvents(){
        self.eventsEdit.removeAll()
        db.collection("Events").order(by: "timeStamp", descending: true).getDocuments { [self] (snap, err) in
            ...
            docs.documentChanges.forEach { (doc) in
                ...
                if self.user?.uid == doc.document.data()["organizerID"] as? String {
                if doc.type == .added{
